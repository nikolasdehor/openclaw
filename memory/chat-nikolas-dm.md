...
- Novo pedido do Nikolas: no grupo FOR6DEVS eu posso interagir com os outros devs diretamente (não só quando o Nikolas mandar mensagem). Manter sem spam: entrar quando eu destravar algo, responder dúvida técnica, ou ajudar em organização/decisão objetiva; se depender de decisão pessoal do Nikolas, perguntar ele antes.
- Nikolas pediu para o WhatsApp watchdog considerar áudio no FOR6DEVS com conteúdo (transcrição), não só marcar como [Áudio]. Limitação: para baixar mídia, o `wacli media download` precisa do store lock e conflita com `wacli sync --follow`; solução proposta: transcrever sob demanda ou pausar o sync rapidamente quando houver áudio e reiniciar.
